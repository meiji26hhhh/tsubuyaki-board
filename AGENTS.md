...
`./mvnw -B -Ph2 verify` が緑で、仕上げ時に `./mvnw -B -Ph2 -Pcoverage-day3 -Pstrict verify` が緑、
Repository / Service / Controller それぞれ 1 本以上のテストがあり、JaCoCo カバレッジ 80% 以上、
`docs/prompts-i-used.md` が提出されている状態。
...
- `src/main/java/com/example/tsubuyaki/` 業務コード
- `src/test/java/com/example/tsubuyaki/sample/` TDD 雛形 (**削除禁止**)
...
**RED → GREEN → REFACTOR** を 1 ユースケース内で完結させる。
...
- 受講生は各自の **fork** の自分専用ブランチ `<github-id>`（GitHub ユーザ名）で作業する。課題ごとに分けたい場合のみ任意で `<github-id>/<課題番号>-<要約>`（例: `yamada/m1-post-list`）のサブブランチを切る。
- `main`（fork の main も upstream も）は配布時のスターターのまま温存する。
- `main` への直接 push は禁止。**push 先は常に自分の fork のブランチ**（`origin <github-id>`）。変更は **fork → upstream への Pull Request**（Draft 可・マージしない・講師レビュー用）で提案する。upstream には push 権限が無い。
...
- **1 コミット群 = 1 ユースケース**。差分 200 行を超える場合はコミットを分割する。
- 自分の fork のブランチへ push した時点で 1 ユースケース完了とする（push で講師レビュー用の PR の差分が更新される）。前提として **ローカル `./mvnw -B -Ph2 verify` が緑**でなければ push しない。
- upstream `main` へはマージしない（`main` はスターターのまま）。受講生は 1 本の `<github-id>` ブランチに課題を積んでいく。

## 4. テスト規約

### 4.1 カバレッジ閾値

JaCoCo の行カバレッジを 3 段階でスライドする (序盤 60% → 中盤 70% → 仕上げ 80%)。プロファイル名 `coverage-dayN` は内部識別子で、フェーズ単位の段階引き上げを示す。

| プロファイル | 閾値 | 想定フェーズ |
|---|---|---|
| 既定（無指定 = `-Pcoverage-day1` 相当） | 60% | 「投稿一覧 (M1)」までの導入フェーズ |
| `-Pcoverage-day2` | 70% | 中盤（M2/M3 完了 → リファクタ手前） |
| `-Pcoverage-day3` | 80% | 「リファクタ＋カバレッジ80%到達」以降、「仕上げ」まで維持 |

```bash
./mvnw -B -Ph2 -Pcoverage-day2 verify   # 中盤 70% gate
./mvnw -B -Ph2 -Pcoverage-day3 -Pstrict verify   # 仕上げ合否 gate
```
...
| `@SpringBootTest` | 統合。最小限。1 ユースケース内で増やしすぎない |
...
研修中は warning 表示。`-Pstrict` で error 扱い (仕上げ・相互レビュー時に推奨)。
...
`.codex/config.toml` の `approval_policy = "on-request"`。
通常は自走し、サンドボックス外の操作が必要なときのみ受講生に承認を求める。
安全最優先の回は講師判断で `untrusted` (信頼済みコマンド以外は毎回確認) に切り替える。

### 7.2 ネットワーク

許可: Maven Central / GitHub / api.openai.com のみ。
それ以外は受講生に確認する。Podman 側の egress を完全に物理制限しているわけではないため、
許可外アクセスを見つけたら即座に中断する。

### 7.3 禁止コマンド

本リストは Codex devbox の研修ハーネス (`containers/codex-devbox/bin/*-guard.sh` +
`scripts/run-codex.sh` のマスクマウント) で多層防御されている。
ただし絶対パスで実体バイナリを直接呼ぶような明示的な迂回はプロンプト層の禁止事項として扱い、
見つけ次第セッションを中断する。ハーネスに頼らずプロンプト層でも遵守する。
...
- `git push --force` / `-f` / `--force-with-lease` (自分のブランチを含め一律)
- `main` への push (`git push origin main` / `--all` / `--mirror` 等。fork の main も upstream も対象)。push 先は常に自分の `<github-id>` ブランチ。研修ハーネスの `git-guard.sh` が main への push を拒否する。upstream へは PR で提案する
...
- フォーマット適用
- 自分の `<github-id>` ブランチへのコミット
- 自分の fork の `<github-id>` ブランチへの push (force ではない通常 push のみ。`main` への push は不可)
...
> 💡 受講生向けの「破壊操作を踏みそうになったときの安全手順」は [education/TROUBLESHOOTING.md の Git 操作の安全ガイド](./education/TROUBLESHOOTING.md#git-操作の安全ガイド) を参照。
...
| 対象 | ドキュメント |
|---|---|
| 受講生: 演習 3 日間の動き方 | [education/ONBOARDING.md](./education/ONBOARDING.md) |
| 受講生: 初期セットアップ手順 | [education/student-setup-guide.md](./education/student-setup-guide.md) |
| 受講生: トラブル対処 | [education/TROUBLESHOOTING.md](./education/TROUBLESHOOTING.md) |
| 講師: 研修準備・運営 | [instructor/instructor-setup-guide.md](./instructor/instructor-setup-guide.md) |
| 講師: タイムテーブル | [instructor/timetable.md](./instructor/timetable.md) |
| 機能要件・受入基準 | [education/EXERCISES.md](./education/EXERCISES.md) |
| Codex 補足指示 | [.codex/instructions.md](./.codex/instructions.md) |
