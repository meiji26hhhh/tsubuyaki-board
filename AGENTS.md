...
- 受講生は自分専用の作業ブランチ `<github-id>`（GitHub ユーザ名）で作業する。課題ごとに分けたい場合のみ任意で `<github-id>/<課題番号>-<要約>`（例: `yamada/m1-post-list`）のサブブランチを切る。
- 共有 `main` は配布時のスターターのまま温存する（誰も更新しない）。
- 共有 `main` への push は禁止。**push 先は常に自分のブランチ**（`origin <github-id>`）。本研修では PR を作らない（全受講生が同じ共有リポを使うため、共有 main へは取り込まない）。
...
- 本研修は **PR を作らない**。自分のブランチへ push した時点で 1 ユースケース完了とする。前提として **ローカル `./mvnw -B -Ph2 verify` が緑**でなければ push しない。
- 共有 `main` へはマージしない（`main` はスターターのまま）。受講生は 1 本の `<github-id>` ブランチに課題を積んでいく。
...
- 共有 `main` への push (`git push origin main` / `--all` / `--mirror` 等)。push 先は常に自分の `<github-id>` ブランチ。研修ハーネスの `git-guard.sh` が main への push を拒否する
...
- 自分の `<github-id>` ブランチへの push (force ではない通常 push のみ。共有 `main` への push は不可)
...
| 機能要件・受入基準 | [EXERCISES.md](./EXERCISES.md) |
