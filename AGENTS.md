...
### 3.4 PR の単位と self-merge

- 1 PR = 1 ユースケース。差分 200 行を超える場合は分割する。
- 研修中は **self-merge を許可**する（受講生は自分の Classroom リポで完結するため）。前提として **ローカル `./mvnw -B -Ph2 verify` が緑**でなければ merge しない。
- マージ後はマージ済みブランチを削除する（同名で次フェーズに使い回さない）。
