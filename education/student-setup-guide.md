...
| **feature ブランチ** | 機能ごとに分けて作業する Git のブランチ。`feature/<your-name>-m1` のような名前で切る。main 直接 push は規約上禁止。必ず feature ブランチで開発する。 |
...
main 直 push は規約上禁止です。**必ず feature ブランチで開発**します。
...
ここまで完了したら、初回 push と品質ゲート（ローカル `verify`）の緑化を確認します。本研修では GitHub Actions による CI を使わず、**ローカルで `./mvnw -B -Ph2 verify` を回して BUILD SUCCESS することを完了基準**とします。
