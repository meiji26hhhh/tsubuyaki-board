...
- インターネット接続
- ディスクの空き容量 **20 GB 以上**（WSL Ubuntu / Podman イメージ / Oracle データで消費します）
...
ここからはubuntuからリポルート（`/mnt/c/workspace/tsubuyaki-board`）で実行します。
...
> ⚠️ PRは講師の指示があるまで出さないでください。最初の準備では、9-3まで完了していればOKです。
...
>
> 💡 研修実施期間外は、tubuyaki-boardのPublicリポジトリはクローズしている場合があります。自習をしたい場合は、forkした自分のリポジトリのmainブランチから新しいブランチを作成してください。
