...
# Oracle 用のキーだけが入っていることを確認（値は表示しない）
grep -E '^(ORACLE_|SPRING_)' .env | cut -d= -f1
```
...
> 💡 `.env` に `OPENAI_API_KEY` は書きません。Codex 用キーは次の §7-2 で `~/.bashrc` にだけ設定します。
...
ファイル末尾に `export OPENAI_API_KEY=<配布されたキー>` を 1 行追記して保存。画面共有中は入力値を表示しないでください。
