...
本研修では PR を作らず、**自分の作業ブランチに push した時点で 1 ユースケース完了**です。共有 `main` には誰も取り込みません（`main` は配布時のスターターのまま温存）。

```bash
# push 前にローカルで verify が緑であることを必ず確認
./mvnw -B -Ph2 verify

# 自分のブランチへ push（§9-2 で -u 済みならブランチ名は省略可）
git push
```

#### コミット前セルフチェック（旧 PR テンプレートの観点）

PR が無くなった代わりに、各ユースケースを push する前に以下の観点で自己点検します。仕上げ（17-19h）と相互レビュー（19-20h）でもこの観点を使います。
...
GitHub 上で `https://github.com/<org>/tsubuyaki-board/compare/main...<github-id>` を開くと、スターター（main）からの自分の全変更が diff で見られます。相互レビュー（19-20h）ではこの **Compare ビュー**を画面共有し、コミットにコメントを付け合います。
...
- 各人 5 分で自分のブランチの **Compare ビュー**（`compare/main...<github-id>`）を画面共有
- 隣の人が 3 分でレビューコメント（機能・テスト・コミットの 3 観点）。コメントは Compare 画面の各コミット／各行に直接付けられる
...
- 共有 `main` ブランチへの push（規約上禁止。push 先は常に自分の `<github-id>` ブランチ）
- 他の受講生のブランチへの push・削除
