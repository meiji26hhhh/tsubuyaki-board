...
⑧  git add / commit (Conventional Commits) / push / PR 作成
...
3. push 前に `./mvnw -B -Ph2 verify` が緑であることを確認しておく。
...
ローカルで `./mvnw -B -Ph2 verify` が緑なことを確認したら、PR 画面で「**Merge pull request**」→「**Confirm merge**」。研修中は self-merge を許可（[AGENTS.md §3.4](../AGENTS.md) 参照）。マージ後はブランチを削除して OK（同名で次フェーズ再利用しない）。
...
- `main` ブランチへの直 push（規約上禁止。必ず feature ブランチ＋PR 経由）
