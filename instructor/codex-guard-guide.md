...
研修では受講生が Codex CLI を **自走（`approval_policy = "on-failure"`）** させて開発する。守りたいものは 3 つ。
...
| `git-guard.sh` | `push --force/-f/--force-with-lease`・`rm -r`・`clean -f系`・`reset --hard`・`checkout -- .`・`restore .`・`config --global/--system`・`filter-branch`・`update-ref --delete` | `add` `commit` `通常 push` `rm <file>`（単発）`restore <file>`（単発）`stash` `status` `log` `diff` |
...
研修の回や受講生のレベルに応じて、講師が調整できる箇所。いずれも `.codex/config.toml` を編集して PR で共有する運用。
