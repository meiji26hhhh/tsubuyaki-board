...
研修では受講生が Codex CLI を **自走（`approval_policy = "on-request"`）** させて開発する。守りたいものは 4 つ。
...
4. **`main` ブランチ** — 受講生は各自の fork で作業し、変更は PR で upstream に提案する。`main`（fork の main も upstream も）を直接汚さない規律を守るため、Codex が `main` へ push しようとするのを止める（[instructor-setup-guide.md §4](./instructor-setup-guide.md) のブランチ保護 + Codex Guard の一翼。Codex Guard はそのうちコンテナ内 Codex の経路を担う）。
...
flowchart TB
    ...
    G --> D{"危険パス /<br/>破壊オプション /<br/>main への push?"}
...
| `git-guard.sh` | `push --force/-f/--force-with-lease`・**`push` の宛先が `main`（fork の main も直接 push させない。`main` checkout 中の refspec なし / `HEAD` の暗黙 push も拒否）**・`rm -r`・`clean -f系`（結合順不問）・`reset --hard`・`checkout -- .`・`restore .`・`config --global/--system`・`filter-branch`・`update-ref --delete` | `add` `commit` `自分のブランチへの通常 push` `rm <file>`（単発）`restore <file>`（単発）`stash` `status` `log` `diff` |
...
| 設定 | 値 | 意味 |
|---|---|---|
| `model` | `gpt-5.5` | 研修標準モデル（2026-06 時点。旧 `gpt-5-codex` / `gpt-5.1-codex` 系は廃止） |
| `approval_policy` | `on-request` | 通常自走、サンドボックス外の操作のみ受講生に確認（§3-4 で `untrusted` に切替可）。旧 `on-failure` は CLI 側で非推奨化 |
...
| `[tools].web_search` | `false` | 現行 CLI は web 検索が既定有効のため明示オフ。研修中の脱線防止（応用課題で `true` に切替可） |

旧版の config.toml にあった `instructions` キー（禁止リストのプロンプト注入）と独自の `[network]` allowlist セクションは、現行 CLI でそれぞれ「予約済みで機能しない」「スキーマ外」のため削除した。プロンプト層の禁止リストは **`AGENTS.md` §7 が正本**（Codex がプロジェクトの AGENTS.md を自動で読み込む）。通信先の制限は L1〜L3 のコンテナ層と規範で担保する。

なお、リポ直下の `.codex/config.toml`（プロジェクトローカル設定）は **trusted なプロジェクトでのみ**読み込まれる。研修コンテナでは `entrypoint.sh` が研修専用 CODEX_HOME に `[projects."/workspace"] trust_level = "trusted"` を自動追記して有効化し、あわせて `OPENAI_API_KEY` から `auth.json` を生成（`codex login --with-api-key`）、`.codex/prompts/` を `$CODEX_HOME/prompts/`（スラッシュコマンド）へ同期する。

> 💡 config.toml の冒頭コメント自身が「本設定は『Codex への約束事 (設定層)』であり、最終的な物理ブロックはコンテナの研修ハーネスで別途行われる。多層防御の最内殻」と明記している。config は約束事、物理ブロックは L1/L2/L3 という役割分担。
...
sequenceDiagram
    ...
    E->>E: codex login --with-api-key（auth.json 生成）
    E->>E: /workspace の trust 付与・prompts 同期
    E->>E: git safe.directory 登録（実体 /usr/bin/git を直呼び）
    E->>U: 起動バナー（認証・ハーネス有効・.env 上書き済）
...
- `Containerfile:95-96`: `/bin/chmod +x entrypoint.sh`（PATH 先頭の guard を避けるため絶対パス）。
- `entrypoint.sh:60-73`: `/usr/bin/git config --global safe.directory`（guard が `config --global` を拒否するため実体直呼び）。
...
**(a) 起動バナーを見る**（最も簡単）
受講生が `codex-shell` でコンテナに入ると、`entrypoint.sh:75-105` のバナーが出る。次の 2 行を確認する。
...
研修の回や受講生のレベルに応じて、講師が調整できる箇所。いずれも `.codex/config.toml` を編集し、講師が upstream リポの `main` に反映する運用（受講生の fork には影響しない）。

| 設定 | 既定 | 切替の意図 |
|---|---|---|
| `approval_policy` | `on-request` | 安全最優先の回は `untrusted`（信頼済みコマンド以外を毎回確認）にする |
| `[tools].web_search` | `false` | 仕上げ・応用課題で外部調査を許す場合のみ `true` |
| `model` | `gpt-5.5` | 軽量・高速に振りたい補助タスクは `gpt-5.4-mini` も選択肢（モデル一覧は公式 Models ページを確認） |
