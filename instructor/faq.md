...
### Q. 1 人だけ Codex が `git push --force main` しようとした

**即決**: AGENTS.md §7.3.3 の禁止コマンド一覧をその場で読ませる。本研修では GitHub branch protection は設定していない方針（[instructor-setup-guide.md §4](./instructor-setup-guide.md)）のため、**物理ブロックは無い**。

代わりに以下の手順で復旧：

1. 講師は GitHub 上で当該リポの `Settings → Branches` から、必要であれば一時的に `Restrict force pushes` を ON にして二次被害を止める。
2. GitHub 上の main に入った不正コミットを確認し、復旧は `git revert <SHA>` の打ち消しコミットで行う。
3. ローカルの作業ツリーは `git stash push -u` で退避し、必要な変更だけ feature ブランチに戻す。
4. force push による履歴書き換えは研修手順として許可しない。AGENTS.md §7.3.3「履歴・権限改ざん禁止」を全体に再共有。
...
**即決**: その PR をブロックし、AGENTS.md の「失敗テストに `@Disabled` を貼って通すのは絶対禁止」を引用。
