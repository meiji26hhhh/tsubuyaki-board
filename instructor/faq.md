...
**即決**: AGENTS.md §7.3.3 の禁止コマンド一覧をその場で読ませる。コンテナ内 Codex の force push は Codex Guard（`git-guard.sh`）が exit 126 で拒否する。さらに fork モデルでは受講生は upstream への push 権限を持たず、upstream `main` は branch protection で**事前にブロック**される（[instructor-setup-guide.md §4](./instructor-setup-guide.md)）。

念のための手順：

1. upstream `main` への影響は無い（受講生に push 権限が無く、ブランチ保護も効いている）。万一受講生が自分の fork の `main` を汚した場合でも影響は本人の fork に閉じるので、慌てず本人の作業ブランチ（`<github-id>`）で作業を続けさせる。fork の main を作り直したい場合は受講生ガイド [TROUBLESHOOTING の「最後の手段」](../education/TROUBLESHOOTING.md) を案内。
2. ローカルの作業ツリーで救いたい変更は `git stash push -u` で退避し、必要な変更だけ自分の `<github-id>` ブランチに戻す。
