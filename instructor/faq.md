...
**即決**: 5 分待っても直らなければ、その受講者を `--install-codex-host` モードに切り替える。

```bash
bash scripts/setup-wsl.sh --install-codex-host
```

WSL ホストに直 install した Codex CLI で進めさせ、後で Podman の不調は休憩中に調査。

### Q. 1 人だけ Oracle XE が 5 分経っても ready にならない

**即決**: その受講者だけ `application-h2.yml` を default にして「投稿一覧」フェーズを始めさせる。
...
**即決**: AGENTS.md の禁止コマンド一覧をその場で読ませる。
ブランチ保護で `allow_force_pushes: false` を入れているので物理的に通らないことを確認。
...
それでも繰り返すなら、その受講者の AGENTS.md にハードコード禁止の例を追記する案を提示。
