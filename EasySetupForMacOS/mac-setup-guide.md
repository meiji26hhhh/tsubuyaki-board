...
| **upstream（アップストリーム）** | Fork 元の研修リポ（`<owner>/tsubuyaki-board`）。あなたの fork から見た「本家」。最新の取り込みや Pull Request の宛先になる。**あなたには upstream への push 権限は無い**。 |
...
本研修では、講師が公開している研修リポ（upstream: `<owner>/tsubuyaki-board`）を、各受講生が**自分のアカウントへ Fork** して使います。Organization への招待や共有リポの clone はありません。あなたは自分の fork を clone し、**自分専用の作業ブランチ**で作業します。講師へのレビュー依頼は **Pull Request**（fork → upstream）で行います（§8）。

### 3-1. 研修リポを Fork する

1. ブラウザで研修リポ（upstream）`https://github.com/<owner>/tsubuyaki-board` を開きます（`<owner>` は講師から共有された所有者名。**公開リポ**なので招待なしで閲覧可）。
2. 右上の「**Fork**」ボタンをクリック。
...
1. ブラウザで**自分の fork** `https://github.com/<github-id>/tsubuyaki-board` を開きます（上部に「forked from `<owner>/tsubuyaki-board`」と出ていれば正しい）。
...
3. 向きを確認: **base** = `<owner>/tsubuyaki-board` の `main`、**compare** = `<github-id>/tsubuyaki-board` の `<github-id>` ブランチ。
...
>
> 📌 `gh auth login` 済みなら `gh pr create --repo <owner>/tsubuyaki-board --base main --head <github-id>:<github-id> --draft --fill` でも作れます。
...
- [ ] 研修リポ `<owner>/tsubuyaki-board` を **Fork** し、自分の fork にアクセスできる
